...
Context API helps solve the problem of prop drilling with large applications, which can be difficult as you have to constantly be adding them to each new child component. Context API provides a system where you can share the values between components without needing to prop drill. You can define them globally and call them when needed.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions hold the regular javascript objects and functions that the store is going to use/need. Reducers are functions that take the current state and action and return the new state. The store is the object that holds the state of the application. The store is known as a single source of truth because it holds the state of the entire application, so when you are updating state it will all be held there.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

redux-thunk is a middleware function that allows us to use asynchronous functions. it changes our action-creators by returning a function from a function, called a thunk. This allows you to dispatch actions asynchronously.

4. What is your favorite state management system you've learned and this sprint? Please explain why!

I really like the Context API. I think it is very straightforward and simple compared to Redux. It is not as efficient as Redux, but it makes creating a global source of state much easier. I have used it before on some applications so that may be why I am more partial to it as well. I like how createContext() is already built in so there is not a huge set up to get it started.
